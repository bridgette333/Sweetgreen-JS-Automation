...
describe("Sweetgreen homepage", () => {
    ...
    it("Should display images and data about menu items", async () => {
        ...
        const productName = await driver.wait(until.elementLocated(By.xpath("//*[@data-test-id='menu-product-name-spicy thai chicken bowl']" +
            "//*[@class='product-name h4 capitalize']"))).getText();
        assert.equal(productName, "Spicy Thai Chicken Bowl");

        // check that calories are right
        const calories = await driver.wait(until.elementLocated(By.xpath("//*[@data-test-id='menu-product-name-spicy thai chicken bowl']" +
            "//span[@class='product-cals']"))).getText();
        assert.equal(calories, "  -  620 Cal");

        // check that description is right
        const productDescription = await driver.wait(until.elementLocated(By.xpath("//*[@data-test-id='menu-product-name-spicy thai chicken bowl']" +
        ...
        const productCost = await driver.wait(until.elementLocated(By.xpath("//*[@data-test-id='menu-product-name-spicy thai chicken bowl']" +
